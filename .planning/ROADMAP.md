...
- [x] 16-01-PLAN.md — catLeveling 完整实现（clickContext + 9 模块 + druidAtk 路由更新）
...
- [x] 17-01-PLAN.md — 基础设施层：SPELL_NAME_TO_ID 映射表 + resolveSpellId + SpellTrace:register spellName 支持 + loadSpellIdMap + catLeveling FF prowling guard (Wave 1)
...
### Phase 19: 改造druidControl逻辑：拆分bash到新方法druidCharge，加强形态验证和技能存在性判断，优化练级流程控制与冲锋体验
...
- [x] 19-01-PLAN.md — druidControl Bash分支删除 + 新建 druidCharge() 距离驱动双分支方法 + combo.lua自检 (Wave 1)

**Wave 2** *(blocked on Wave 1 completion)*

- [x] 19-02-PLAN.md — selftest.lua Category M 自检注册（4项 druidCharge/druidControl 验证） (Wave 2)

### Phase 20: 添加 SPELL_ID_AUTO_CORRECT 全局开关控制 spellId 自动修正机制
...
- [x] 21-01-PLAN.md — Fix comment numbering (0-12) in combo.lua + add KillShot design intent comments (Items 1+2)
...
### Phase 26: 新增phase以支持猫德的fast战斗逻辑
...
**Depends on:** Phase 25
**Plans:** 3/3 plans executed (2 executed + 1 gap-closure from code review/verification)
...
- [x] 26-02-PLAN.md — 剩余 4 个 Category P SelfTest（HRPS/血量估算/优先级关系/cp5Bite 回归）+ 全阶段验证 (Wave 2)

**Wave 3** *(gap closure on 26-REVIEW.md / 26-VERIFICATION.md)*
...
### Phase 28: catAtk claw/shred/bite damage instrumentation — opt-in combat log + offline analyzer
...
- [x] 28-01-PLAN.md — tracer 切片：cpDamageLog 开关 + JSON 编码器 + batch 戳 + claw 端到端（采样/独立 intent/SELF_DAMAGE 通道门/配对/[cpDamage] 发射）+ tools/cpdamage.lua 骨架 (Wave 1)

**Wave 2** *(blocked on Wave 1)*

- [x] 28-02-PLAN.md — shred/bite 采样同构挂接 + Category U SelfTest 9 条（U-01..U-09） (Wave 2)
- [x] 28-03-PLAN.md — 完整离线分析器：提取/沙箱 + 解码校验 + 四档桶/OOC/bite 最小二乘 + 报表/决策建议/--json-out/--selftest (Wave 2)
...
**Requirements**: TBD
**Depends on:** Phase 28
**Plans:** 4/4 plans executed — 3/3 executed + 29-04 gap closure (UAT G-29-3, resolved 2026-09-12；29-UAT.md complete 5 passed / 0 issues)
...
- [x] 29-02-PLAN.md — 反推兜底层复活：maintainLandTables + computeLandTable 窗口化推断（蓝色 (inferred) 通告、静默窗 ttl 触发、窗口化 fail 否决）+ Q-09 复活断言 + Q-11/Q-13 边界用例

**Wave 3** *(blocked on Wave 2)*

- [x] 29-03-PLAN.md — 边界用例后半程 Q-12/Q-14/Q-15/Q-16 + HUMAN-UAT.md Phase 29 实机验收协议（含 D-17/D-18 注记）+ 全阶段静态收尾电池

**Wave 4** *(gap closure — UAT G-29-3)*

- [x] 29-04-PLAN.md — G-29-3 gap closure：修正 interface_debug.lua macroTorch.show 绿/蓝两臂色相颠倒（渲染层共享通路，D-14 协议） + Category T-02 映射层回归自测 + 静态电池
...
- [x] 30-01-PLAN.md — DKI live 计时器 tracer 切片：三态状态机 + 0.1s 轮询 + [cpBuildT] 发射 + 全局重置双钩子 (Wave 1)
- [x] 30-03-PLAN.md — tools/cpbuild.lua 自包含离线分析器（harness 复制 + terse 解析 + 统计 + --selftest/--json-out/--rake-dur）(Wave 1)

**Wave 2** *(blocked on Wave 1)*

- [x] 30-02-PLAN.md — Category S-05..S-12 stubbed 单测 + HUMAN-UAT.md Phase 30 段（D-09 前置条件）+ 全阶段静态收尾电池 (Wave 2)
