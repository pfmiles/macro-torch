...
-- Rogue combat rotation functions
...
--- 释放技能前先偷东西，需潜行且目标存在
---@param spell string 偷东西后要释放的技能
-- pickPocketBeforeCast: state machine with variable spell names, deferred CastSpellByName migration
-- Only the hardcoded "偷窃" is replaced with player.pick_pocket(); CastSpellByName(spell) remains as-is
-- since 'spell' is a variable parameter (could be any opener)
...
-- lockNearestEnemyThenCast: generic utility with variable spell names, deferred CastSpellByName migration per CONTEXT.md
